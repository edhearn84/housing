---
title: "Nominal House Prices in Luxembourg - Data cleaning"
author: "Eddie Hearn"
date: "`r Sys.Date()`"
---
...
This data is downloaded from the Luxebourgish [Open Data Portal](https://data.public.lu/fr/datasets/prix-annonces-des-logements-par-commune/)
(the data set called the data set called *Série rétrospective des prix
annoncés des maisons par commune, de 2010 à 2021*), and the original data is from the "Observatoire de l'habitat". This data contains prices for houses sold since 2010 for each luxembourgish commune.
...
We need to clean the data: "Luxembourg" is "Luxembourg-ville" in 2010 and 2011, then "Luxembourg". "Pétange" is also spelt non-consistenly, and we also need to convert columns to the right type. We also directly remove rows where the locality contains information on the "Source": 
...
#' @details Removes unneeded rows, renames localities to make their names consistent across years and converts columns with prices to numeric columns
...
We now make sure that we got all the communes/localities in there. There were mergers in 2011, 2015, and 2018. So we need to account for these localities. 
...
Test to see if all the communes from our dataset are represented.
...
Keep the national average in another dataset:
...
Can finish cleaning the commune data:
...
Now save the dataset in a folder for further analysis (keep chunk option to `eval = FALSE` to avoid running it when knitting):
